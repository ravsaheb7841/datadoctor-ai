...
const CleaningCenter = () => {
  ...
  const fetchColumnTypes = useCallback(async () => {
    try {
      const response = await fetch(
        `${API_URL}/api/datasets/${id}/column-types`,
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
  ...
  const getMethodsForIssue = (issue) => {
    ...
    const issueText =
      `${issue.issue || ''} ${issue.type || ''}`.toLowerCase();

    if (issueType === 'high_cardinality') {
      return [];
    }
    ...
    if (byIssue) {
      return (
        byIssue[type] ||
        byIssue.default ||
        getAllowedMethods(issue.column)
      );
    }
  ...
  const handleClean = async (issue, index) => {
    ...
    if (selectedOp === 'merge_categories') {
      if (
        ...
        setError(
          'Enter categories to merge and the new category name'
        );
  ...
  if (loading) {
    return (
      <LoadingState
        message="Loading cleaning center..."
      />
    );
  }
        ...
        {issues.length === 0 ? (
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            {issues.map((issue, index) => {
              ...
              const colType =
                getColumnType(issue.column);

              const allowedMethods =
                getMethodsForIssue(issue);

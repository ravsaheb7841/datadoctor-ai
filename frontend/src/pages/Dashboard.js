...
const Dashboard = () => {
  ...
  const fetchDatasets = async () => {
    try {
      ...
      const totalRows = datasetsList.reduce(
        (sum, ds) => sum + (ds.rows || 0),
        0
      );
      ...
      const avgHealth =
        total > 0
          ? Math.round(
            datasetsList.reduce(
              (sum, ds) => sum + (ds.health_score || 0),
              0
            ) / total
          )
          : 0;

      const totalIssues = datasetsList.reduce(
        (sum, ds) => sum + (ds.issue_count || 0),
        0
      );
      ...
      setStats({
        ...
        avgHealth,
        totalIssues,
        cleanedDatasets: datasetsList.filter(
          ds => (ds.version || 1) > 1
        ).length
      });
    } catch (err) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          return (
            ...
            >
              <div
                className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${colorMap[item.color]} opacity-10 rounded-full -translate-y-1/2 translate-x-1/2 group-hover:scale-150 transition-transform duration-500`}
              ></div>

              <div
                className={`inline-flex p-2.5 rounded-xl bg-gradient-to-br ${colorMap[item.color]} text-white shadow-lg mb-4`}
              >
              ...
              return (
                ...
                >
                  ...
                  <div className="hidden md:flex items-center gap-3">
                    ...
                    <span
                      className={`px-2.5 py-1 rounded-full text-xs font-medium ${ds.status === 'completed'

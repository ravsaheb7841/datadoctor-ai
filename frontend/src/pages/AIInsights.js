...
import {
  ...
  RefreshCw, Copy, CheckCircle2, ChevronDown, ChevronUp,
  ArrowRight, Target, ShieldCheck, Star, Award, BarChart3
} from 'lucide-react';
...
const AIInsights = () => {
  ...
  const fetchInsights = useCallback(async (isRefresh = false) => {
    ...
    setError('');
    try {
      const response = await fetch(`${API_URL}/api/datasets/${id}/insights`, {
  ...
  const SectionHeader = ({ icon, title, subtitle, color, sectionKey, count, children }) => {
    const Icon = icon;
    ...
    <div className="animate-fade-in space-y-6">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">AI Business Insights</h1>
              <p className="mt-1.5 text-purple-200 text-sm max-w-lg">Intelligent analysis, key findings and hidden relationships</p>
            </div>
          </div>
          <button
      ...
      {!hasContent ? (
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 py-16 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">No insights available yet</h3>
          <p className="text-gray-500 dark:text-gray-400 max-w-sm mx-auto">Insights will appear here once the analysis is complete.</p>
        </div>
      ) : (
        <>
            ...
            >
              <div className="p-5 space-y-3">
                {insights.key_findings.map((finding, index) => {
                  const ImportanceIcon = importanceIcons[finding.importance] || Target;
                  return (
                    <div
                      key={index}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-gray-900 dark:text-white font-medium leading-relaxed">{finding.finding}</p>
                        <div className="flex items-center gap-2 mt-2">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize border ${importanceStyles[finding.importance] || importanceStyles.medium}`}>
                ...
                {insights.relationships.map((rel, index) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <p className="text-gray-900 dark:text-white font-medium leading-relaxed flex-1">{rel.finding}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { icon: Lightbulb, label: 'Key Findings', value: insights?.key_findings?.length || 0, color: 'text-amber-500' },
              { icon: Link2, label: 'Relationships', value: insights?.relationships?.length || 0, color: 'text-indigo-500' },
              { icon: BarChart3, label: 'Data Points', value: insights?.data_points || 'N/A', color: 'text-emerald-500' },
            ].map((item, i) => {
